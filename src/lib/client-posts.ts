  ...
  try {
    ...
    if (allPosts.length === 0 || isCacheStale) {
      try {
        ...
        const url = `/api/releases/rss-data-${todayYear}/${today}.json`;
        ...
        if (!response.ok) {
          ...
          if (response.status === 404) {
            ...
            const yesterdayUrl = `/api/releases/rss-data-${yesterdayYear}/${yesterday}.json`;
...
export const fetchArchivePosts = async (date: string): Promise<Post[]> => {
  ...
  try {
    ...
    const url = `/api/releases/rss-data-${year}/${date}.json`;
    const response = await fetch(url);
    if (!response.ok) {
