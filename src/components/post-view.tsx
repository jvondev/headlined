    ...
    <Card className="h-full w-full flex flex-col overflow-hidden relative">
      {showSummary && summaries.length > 0 ? (
        ...
        <>
          {post.thumbnail_url && (
            <div
              className="relative w-full h-3/5 overflow-hidden flex-shrink-0" // Adjusted height
            ...
            >
              ...
              />
              <div className={cn("absolute inset-0", theme === 'light' ? 'bg-white/40' : 'bg-black/40')} />
            </div>
          )}

          <div
            className={cn(
              "relative flex flex-col justify-center items-center p-6 md:p-8 text-center flex-grow", // flex-grow to take remaining space
              "bg-muted", // Muted background color
              post.thumbnail_url && (theme === 'light' ? 'text-black' : 'text-white')
            )}
            onClick={handleTitleClick} // Make the title area clickable
          >
            <div className="max-w-3xl flex flex-col items-center">
              <h1 className="font-headline text-2xl md:text-4xl font-bold mt-2 leading-tight">
                {post.title}
              </h1>
              {post.source_name && (
            ...
            <Button
              className="mt-6 px-6 py-3 text-lg font-semibold rounded-full"
